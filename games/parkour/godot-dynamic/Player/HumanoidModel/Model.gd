...
func _ready():
	...
	# The source scene left the animation players on their editor preview
	# pose until the first state transition. Apply the actual idle state now
	# so a newly spawned parkour character starts in a neutral stance.
