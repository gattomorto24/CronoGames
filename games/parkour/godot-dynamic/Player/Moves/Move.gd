...
# This controller was authored and timed around standard Earth gravity.
# Crono's legacy C++ prototype uses a heavier global value, so keep the
# imported move set self-contained instead of shortening every jump.
